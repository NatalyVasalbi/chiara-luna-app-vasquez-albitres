...
function ItemDetail({ item, initialValue, restar, sumar, message}) {
    ...
    window.addEventListener('clickToCart',(e)=>{
        console.log(e)
        context.addItem({name: item.title, price: item.price, quantity: initialValue})
    })
            ...
            {item.length?? (
                <Row style={{justifyContent: "center"}}>
                    ...
                    <Col md={5} lg={4}>
                        ...
                        <CounterButton initialValue={initialValue} stockItem={6} restar={restar} sumar={sumar}></CounterButton>
                         {message && <Modal />}                        
                    </Col>
                </Row>
